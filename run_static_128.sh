...
OUTPUT=/home/amanda/hemem/data/static_128
...
central_pid=$!
sleep 30
nice -20 numactl -N0 -m0 --physcpubind=14-23 -- env MISS_RATIO=1.0 REQ_DRAM=0 START_CPU=14 LD_PRELOAD=${HEMEM}/src/libhemem.so ${HEMEM}/microbenchmarks/gups-pebs 8 0 38 8 36 1 ${OUTPUT}/perf/bggups.txt > ${OUTPUT}/perf/bggups-setup.txt &
bggups_pid=$!
...
central_pid=$!
sleep 30
nice -20 numactl -N0 -m0 --physcpubind=14-23 -- env MISS_RATIO=1.0 REQ_DRAM=0 OMP_THREAD_LIMIT=8 LD_PRELOAD=${HEMEM}/src/libhemem.so ${HEMEM}/apps/gapbs/bc -n 50 -g 29 > ${OUTPUT}/perf/gapbs.txt &
gapbs_pid=$!
...
central_pid=$!
sleep 30
nice -20 numactl -N0 -m0 --physcpubind=14-23 -- env MISS_RATIO=1.0 REQ_DRAM=0 OMP_THREAD_LIMIT=8 LD_PRELOAD=${HEMEM}/src/libhemem.so ${HEMEM}/apps/nas-bt-c-benchmark/NPB-OMP/bin/bt.E -n 50 -g 28 > ${OUTPUT}/perf/bt.txt &
bt_pid=$!
