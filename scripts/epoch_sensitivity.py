...
infile = open("results/epoch_" + time + "_miss_ratios.txt", encoding='utf8')
lines = infile.readlines()
for line in lines:
    words = line.split()
    miss_ratios.append(words[1])

infile = open("results/epoch_" + time + "_flexkv_1.txt", encoding='utf8').read()
# Throughput of app
...
infile = open("results/epoch_" + time + "_flexkv_2.txt", encoding='utf8').read()
matches = tput_pattern.findall(infile)
for match in matches:
    match = re.compile(r"[0-9]+\.[0-9]+").search(match)
    if match:
...
outfile = open("epoch_" + time + "_summary_miss_ratios.out", "w", encoding='utf8')
for miss in miss_ratios_clipped:
    outfile.write(miss)
    outfile.write("\n")

outfile = open("epoch_" + time + "_summary_throughput.out", "w", encoding='utf8')
for tput in throughput:
    outfile.write(str(tput) + "\n")
outfile.close()

outfile = open("epoch_" + time + "_summary_latencies.out", "w", encoding='utf8')
for i in range(MAX_LATENCY):
