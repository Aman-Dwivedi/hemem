...
OUTPUT=/home/amanda/hemem/data/static_0
...
central_pid=$!
sleep 30
nice -20 numactl -N0 -m0 --physcpubind=4-13 -- env MISS_RATIO=1.0 REQ_DRAM=0 LD_PRELOAD=${HEMEM}/src/libhemem.so ${HEMEM}/apps/flexkvs/kvsbench -t 4 -T ${RUNTIME} -w ${WARMUP} -h ${HOTFRAC1} 127.0.0.1:11211 -S ${FLEXKV_SIZE} > ${OUTPUT}/perf/flexkvs-isolated.txt &
flexkvs_pid=$!
...
nice -20 numactl -N0 -m0 --physcpubind=4-13 -- env MISS_RATIO=1.0 REQ_DRAM=0 LD_PRELOAD=${HEMEM}/src/libhemem.so ${HEMEM}/apps/flexkvs/kvsbench -t 4 -T ${RUNTIME} -w ${WARMUP} -h ${HOTFRAC1} 127.0.0.1:11211 -S ${FLEXKV_SIZE} > ${OUTPUT}/perf/flexkvs-gups.txt &
flexkvs_pid=$!
...
nice -20 numactl -N0 -m0 --physcpubind=4-13 -- env MISS_RATIO=1.0 REQ_DRAM=0 LD_PRELOAD=${HEMEM}/src/libhemem.so ${HEMEM}/apps/flexkvs/kvsbench -t 4 -T ${RUNTIME} -w ${WARMUP} -h ${HOTFRAC1} 127.0.0.1:11211 -S ${FLEXKV_SIZE} > ${OUTPUT}/perf/flexkvs-gapbs.txt &
flexkvs_pid=$!
...
nice -20 numactl -N0 -m0 --physcpubind=4-13 -- env MISS_RATIO=1.0 REQ_DRAM=0 LD_PRELOAD=${HEMEM}/src/libhemem.so ${HEMEM}/apps/flexkvs/kvsbench -t 4 -T ${RUNTIME} -w ${WARMUP} -h ${HOTFRAC1} 127.0.0.1:11211 -S ${FLEXKV_SIZE} > ${OUTPUT}/perf/flexkvs-bt.txt &
flexkvs_pid=$!
